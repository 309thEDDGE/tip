...
1. ch10 files in the truth directory and not included in the ch10list are not processed or mentioned in the report
2. raw/translated/video pq present in the test directory and not included in the ch10list are ignored and do not interfere with the test nor add information to the log
3. raw/translated/video pq present in the truth directory and not included in the ch10list are ignored
4. xxxxx extra files generated by a newer version of TIP relative to the truth set do not interfere with the validation (ex: _ethernet.parquet, additional yaml or txt metadata 
files within translated data dirs must be exclusively ignored in the PqPqTranslated1553DirValidation class)
5. ch10 files included in ch10list but not present in truth are recorded as null and total pass is null
6. correct and present ch10 files included in ch10list paired with icd which does not exist also results in null for the ch10 and null total pass
7. translated 1553 msg dirs that exist in test data but contain no pq files for comparison show null results
8. translated 1553 msg dirs that do not exist in test data are reported as null
9. parse and translate durations are recorded as None if the executable was not run, either by error or because the output files already exist in the test directory
10. metadata files that are present in truth data and do not exist in test data report as null, cause a null result for the total ch10
11. metadata files that are present in truth data and exist in test data and do not compare exactly are reported as failed, unless, as in yaml files, there are sub-level components which are compared individually, in which case the logic given above applies
12. video pq files are only compared when the -v flag is passed to end_to_end_validator.py
'''
