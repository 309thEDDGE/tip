...
void Ch10Time::ParseBinaryWeightedTime(const uint8_t*& data, uint64_t& time_ns)
{
    ...
    high_order_ns_ = uint64_t(binwt_time_ptr_->high_order * double(655.36)) * 1e9;

    // Multiple by 0.01 second per bit, then 1e9 to get nanosecond.
    low_order_ns_ = uint64_t(binwt_time_ptr_->low_order * double(0.01)) * 1e9;

    // Calculate nanoseconds from microsecond word.
    time_ns = uint64_t(binwt_time_ptr_->microsec) * 1e3;
