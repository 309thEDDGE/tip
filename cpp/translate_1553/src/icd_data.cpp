...
yaml_msg_data_keys_({ "lru_addr", "lru_subaddr", "lru_name", "bus", "wrdcnt", "rate", "mode_code", "desc" }),
yaml_word_elem_keys_({"off", "cnt", "schema", "msbval", "uom", "multifmt", "class"}),
yaml_bit_elem_keys_({ "off", "cnt", "schema", "msbval", "uom", "multifmt", "class", "msb", "lsb", "bitcnt" })
{
	...
	std::vector<ICDElement>& icd_elems_output)
{
	...
	if (col_hdr.size() != ICDElement::kFillElementCount)
	{
		...
		printf("ICDData::IngestICDTextFileLines(): first line column count "
	...
	{
		printf("ICDData::IngestICDTextFileLines(): the first two column header "
	...
	uint8_t dest_lru_subaddr)
{
	try
	{
		temp_table_inds_ = icd_temp_lookup_[bus_name][xmit_lru_addr][
			dest_lru_addr][xmit_lru_subaddr][dest_lru_subaddr];
	}
	...
	uint8_t dest_lru_addr, uint8_t xmit_lru_subaddr, uint8_t dest_lru_subaddr)
{
	try
	{
		temp_table_inds_ = icd_lookup_[chanid][xmit_lru_addr][
			dest_lru_addr][xmit_lru_subaddr][dest_lru_subaddr];
	}
	catch (const std::out_of_range& oor)
	{
	...
	std::vector<ICDElement>& icd_elems_output)
{
	...
	for (YAML::const_iterator it = root_node.begin(); it != root_node.end(); ++it)
	{
		...
		{
			printf("ICDData::IngestICDYamlFileLines(): Message %s body is not a map "
		...
		{
			printf("ICDData::IngestICDYamlFileLines(): Message %s msg_data node "
	...
	const YAML::Node& elem_node, bool is_bit_node, std::vector<ICDElement>& icd_elems)
{
	...
	if (!elem_node.IsMap())
	{
		printf("ICDData::FillElementFromYamlNodes(): Message %s elem node "
			"is not a map!\n", msg_name.c_str());
		return fill_count;
	}
	...
	std::vector<std::string>& output_vec)
{
	...
	{
		printf("ICDData::CreateVectorOfStringICDComponents(): %s/%S msg_data[command] node "
	...
	{
		printf("ICDData::CreateVectorOfStringICDComponents(): %s/%S msg_data[lru_addr] node "
			"is not a sequence or does not have size = %zu!",
			msg_name.c_str(), elem_name.c_str(), icd_sequence_size_);
		return false;
	}
	...
	{
		printf("ICDData::CreateVectorOfStringICDComponents(): %s/%S msg_data[lru_subaddr] node "
			"is not a sequence or does not have size = %zu!",
			msg_name.c_str(), elem_name.c_str(), icd_sequence_size_);
		return false;
	...
	{
		printf("ICDData::CreateVectorOfStringICDComponents(): %s/%S msg_data[lru_name] node "
			"is not a sequence or does not have size = %zu!",
			msg_name.c_str(), elem_name.c_str(), icd_sequence_size_);
		return false;
	}
	...
	output_vec[17] = std::to_string(int(elem_data_node["multifmt"].as<bool>()));
