...
Translatable1553Table::Translatable1553Table(std::string msg_name, ICDData* icd, size_t icd_table_index)
    : row_group_size_(0), name_(msg_name), column_count_(0), col_ind_(0), source_raw_data_ptr_(nullptr), translation_err_count_(0), temp_wordno_(0), temp_bitmsb_(0), icd_schema_(ICDElementSchema::BAD), temp_is_bitlevel_(false), temp_byte_count_(0), allocated_row_count_(0), pq_(nullptr), temp_name_(""), bad_value_(0), icd_(icd), icd_translate_(), icd_table_index_(icd_table_index)
{
}
...
void Translatable1553Table::append_data(int64_t& time, const int32_t* raw_data, int64_t& row_ind, bool debug)
{
    ...
    // TODO(ijmyers): Will need to have switch logic at some point to handle multiple ridealong columns
