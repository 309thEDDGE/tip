...
std::string BusMap::PrintFinalMap()
{
    ...
    if (iterable_tools_.GetKeys(final_bus_map_with_sources_).size() != channel_ids_.size())
        ss << "\n--Unmapped (reason)--\n";
    else
...
bool BusMap::UserAdjustments(std::vector<std::string>* test_options)
{
    ...
    while (1)
    {
        ...
        else if (user_input == "2")
        {
            ...
            std::string prompt = "Enter a Bus Name For Channel ID " + std::to_string(channel_id_input) + ":";
    ...
    size_t submission_size)
{
    ...
    {
        key_ = ((transmit_cmd[i] << 16) | (recieve_cmd[i])) & mask_;
...
{
    key_ = ((transmit_cmd << 16) | (recieve_cmd)) & mask_;
    if (icd_message_key_to_channelids_map_.count(key_) == 1)
    {
    ...
    if (vote_method_checks_tmats_)
    {
        for (std::map<uint64_t, std::string>::iterator it = vote_map.begin();
            ...
            else
            {
                ...
                if (tmats_chanid_to_source_map_[it->first].find(it->second) == std::string::npos &&
                                 ...
                                 std::string source)
{
    ...
    for (std::map<uint64_t, std::string>::const_iterator
             it = insert_map.begin();
         it != insert_map.end(); it++)
    {
        if (final_bus_map_with_sources_.count(it->first) == 0 && channel_ids_.count(it->first) > 0)
        {
