...
TranslationMaster::TranslationMaster(const ManagedPath& input_parquet_path,
                                     ...
                                     bool select_msgs, std::vector<std::string> select_msg_names, ICDData icd) : n_threads_(n_threads), parquet_path_(input_parquet_path), worker_wait_(200), worker_start_offset_(2000), is_multithreaded_(true), output_base_path_(), output_base_name_(), msg_list_path_(), parquet_path_is_dir_(false)
{
...
uint8_t TranslationMaster::translate()
{
    ...
    else
    {
        ...
        uint8_t files_per_thread = static_cast<uint8_t>(ceil(static_cast<float>(input_path_count) / n_threads_));
            ...
            for (int i = 0; i < files_per_thread; i++)
            {
                ...
                if (counter == input_path_count)
                {
                    ...
                    if (thread_index < n_threads_ - 1)
                    {
                        ...
                        printf(
                            "All files have been distributed to first %d threads. "
