...
TranslatableColumn<T>::TranslatableColumn(const ICDElement* icd_elem_ptr, bool is_ride_along,
	std::shared_ptr<arrow::DataType> arrow_type, const int& row_alloc_count) : TranslatableBase(
		icd_elem_ptr, is_ride_along, arrow_type), translated_data_ref_(translated_data_)
{
