	...
	std::string format_spec, std::string delim)
{
	...
	for (typename T::const_iterator it = input_iterable.begin(); it != input_iterable.end(); ++it)
	{
		if (i == 0)
		{
			sprintf(buff, format_spec.c_str(), *it);
		}
		else
		{
			sprintf(buff, format_and_delim.c_str(), *it);
		}
		ret_val += buff;
		i++;
	}
	...
	const std::vector<T>& input_iterable2)
{
	...
	size_t output_size = size_t((in1_sorted.size() > in2_sorted.size())
		? in1_sorted.size() : in2_sorted.size());
	...
	const std::vector<C>& input, const F(C::* field))
{
	...
	// TODO: Should we return a map of the unique member values to vectors of indices in
