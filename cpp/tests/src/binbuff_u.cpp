...
protected:
	...
	BinBuffTest() : bb_(), result_read_size_(UINT64_MAX), seek_file_size_(UINT64_MAX),
		temp_file_name_("binbuff_test_temp_file.bin"), write_size_(0), requested_read_pos_(0) {}
	/*void SetUp() override
	{
...
TEST_F(BinBuffTest, FindPattern2ndOccurrence)
{
	...
	requested_read_size_ = 500;
	EXPECT_EQ(bb_.Initialize(infile_, seek_file_size_,
		requested_read_pos_, requested_read_size_), write_size_ - requested_read_pos_);
