...
class NetworkPacketParserTest : public ::testing::Test
{
   protected:
    ...
    NetworkPacketParserTest() : npp_(), eth_data_(), data_length_(0), result_(false), ethertype_(0), mock_npp_top_level_(), npp_ptr_(nullptr), mock_npp_sec_level_(), pdu_ptr_(nullptr), mock_npp_selector_(), channel_id_(12)
    {
    }
...
TEST_F(NetworkPacketParserTest, ParseEthernetHeaderData)
{
    ...
    ON_CALL(mock_npp_selector_, ParserSelector(_, _)).WillByDefault(Return(true));
...
TEST_F(NetworkPacketParserTest, ParseEthernetIIParseIPv4)
{
    ...
    EXPECT_CALL(mock_npp_sec_level_, ParseIPv4(_, &eth_data_)).Times(1).WillOnce(Return(true));
...
TEST_F(NetworkPacketParserTest, ParseEthernetParseEthernetLLC)
{
    ...
    EXPECT_CALL(mock_npp_sec_level_, ParseEthernetLLC(_, &eth_data_)).Times(1).WillOnce(Return(true));
...
TEST_F(NetworkPacketParserTest, ParserSelectorParseEthernetLLC)
{
    ...
    EXPECT_CALL(mock_npp_sec_level_, ParseEthernetLLC(&llc, &eth_data_)).Times(1).WillOnce(Return(true));
...
TEST_F(NetworkPacketParserTest, ParserSelectorParseRaw)
{
    ...
    EXPECT_CALL(mock_npp_sec_level_, ParseRaw(&raw, &eth_data_, _)).Times(1).WillOnce(Return(true));
...
TEST_F(NetworkPacketParserTest, ParserSelectorParseIPv4)
{
    ...
    EXPECT_CALL(mock_npp_sec_level_, ParseIPv4(&ip, &eth_data_)).Times(1).WillOnce(Return(true));
...
TEST_F(NetworkPacketParserTest, ParserSelectorParseUDP)
{
    ...
    EXPECT_CALL(mock_npp_sec_level_, ParseUDP(&udp, &eth_data_)).Times(1).WillOnce(Return(true));
...
TEST_F(NetworkPacketParserTest, ParserSelectorParseTCP)
{
    ...
    EXPECT_CALL(mock_npp_sec_level_, ParseTCP(&tcp, &eth_data_)).Times(1).WillOnce(Return(true));
...
TEST_F(NetworkPacketParserTest, CreateSpecificPcapPathPDUTypes)
{
    // Create fake input path.
    ManagedPath pq_base_path;
    pq_base_path = pq_base_path / std::string("mydata_ethernet.parquet") /
    ...
    // Expected path
    channel_id_ = 13;
    Tins::PDU::PDUType pdu_type = Tins::PDU::PDUType::DOT3;
    ManagedPath expected = pcap_base_path.parent_path().CreatePathObject(
        pcap_base_path, "_IEEE802_3_chanid13.pcap");

    ManagedPath observed = npp_.CreateSpecificPcapPath(pcap_base_path, channel_id_,
                                                       pdu_type);
    EXPECT_EQ(expected.RawString(), observed.RawString());

    channel_id_ = 8;
    pdu_type = Tins::PDU::PDUType::ETHERNET_II;
    expected = pcap_base_path.parent_path().CreatePathObject(
        pcap_base_path, "_ETHERNET_II_chanid08.pcap");

    observed = npp_.CreateSpecificPcapPath(pcap_base_path, channel_id_,
                                           pdu_type);
    EXPECT_EQ(expected.RawString(), observed.RawString());
}

TEST_F(NetworkPacketParserTest, AddPacketWriterToMapChannelIDNotPresent)
{
    // Create fake input path.
    ManagedPath pq_base_path;
    pq_base_path = pq_base_path / std::string("mydata_ethernet.parquet") /
    ...
    // Tins::PacketWriter checks for a valid path on object creation, not
    // when the first packet is written. The base dir must exist.
