...
TEST_F(Ch10ParseMainTest, StartParseConfigureFalse)
{
    ...
    ASSERT_FALSE(StartParse(input_path_, out_path_, config, duration, &mock_pm_));
}
...
TEST_F(Ch10ParseMainTest, StartParseRecordMetadataFalse)
{
    ...
    EXPECT_CALL(mock_pm_, RecordMetadata(input_path_, _)).WillOnce(Return(false));
...
TEST_F(Ch10ParseMainTest, StartParseTrue)
{
    ...
    EXPECT_CALL(mock_pm_, Configure(input_path_, out_path_, _)).WillOnce(Return(true));
    EXPECT_CALL(mock_pm_, Parse(_)).WillOnce(Return(true));
    EXPECT_CALL(mock_pm_, RecordMetadata(input_path_, _)).WillOnce(Return(true));

    ASSERT_TRUE(StartParse(input_path_, out_path_, config, duration, &mock_pm_));
}